...
export function resolveDajuSkuStockBinding(productMetadata, skuMetadata, allowLegacyProductFallback = false) {
  const directSkuBinding = parseDajuProductBinding(isPlainRecord(skuMetadata) ? skuMetadata : {});
  if (directSkuBinding) return directSkuBinding;
  const skuBinding = parseDajuSkuStockBinding(productMetadata, skuMetadata);
  const hasExplicitSupplierSku = isPlainRecord(skuMetadata)
    && Object.prototype.hasOwnProperty.call(skuMetadata, "supplier_sku");
  if (hasExplicitSupplierSku || !allowLegacyProductFallback) return skuBinding;
  return parseDajuProductBinding(isPlainRecord(productMetadata) ? productMetadata : {});
}
