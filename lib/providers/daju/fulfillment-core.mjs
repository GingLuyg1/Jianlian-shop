...
function safeError(error) {
  if (error && typeof error === "object" && typeof error.code === "string") {
    return { code: error.code, kind: typeof error.kind === "string" ? error.kind : "http" };
  }
  return { code: "UPSTREAM_UNAVAILABLE", kind: "transport" };
}
  ...
  for (const candidate of input.candidates) {
    ...
    if (claim.action === "QUERY") {
      ...
      try {
        ...
        const decision = decideDajuError(safeError(error));
        await record(input.store, input.outcome(candidate, claim, requestId, decision.state, decision.retryable, decision.code, { supplierUnitPrice: product.price }));
