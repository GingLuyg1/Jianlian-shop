...
export function validateDajuBindingAgainstProductDetail(binding, product) {
  if (!binding || !isPlainRecord(product) || product.id !== binding.productId) {
    return { ok: false, code: "DAJU_PRODUCT_MISMATCH", missing: [] };
  }
  if (product.isAuto !== true) {
    return { ok: false, code: "DAJU_PRODUCT_NOT_AUTOMATIC", missing: [] };
  }
  if (!Array.isArray(product.requiredInputs) || !isPlainRecord(binding.inputsMapping)) {
    return { ok: false, code: "DAJU_PRODUCT_DETAIL_INVALID", missing: [] };
  }

  const missing = [];
  for (const required of product.requiredInputs) {
    if (typeof required !== "string" || !/^[a-zA-Z][a-zA-Z0-9_]{0,79}$/.test(required)) {
      return { ok: false, code: "DAJU_PRODUCT_DETAIL_INVALID", missing: [] };
    }
    const source = binding.inputsMapping[required];
    if (typeof source !== "string" || !INPUT_SOURCES.has(source)) missing.push(required);
  }
  return missing.length > 0
    ? { ok: false, code: "DAJU_REQUIRED_INPUTS_UNMAPPED", missing }
    : { ok: true, missing: [] };
}
