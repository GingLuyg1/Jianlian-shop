...
export async function fulfillSupplierCandidates(input) {
  ...
  for (const candidate of input.candidates) {
    ...
    if (claim.action === "QUERY") {
      ...
      try {
        ...
        const decision = input.decideError(safe);
    ...
    let decision = input.decideOrderResult(orderResult);
    if (decision.kind === "query_order") {
      try {
        orderResult = await input.client.getOrder(decision.orderCode);
        decision = input.decideOrderResult(orderResult);
      } catch (error) {
    ...
    if (decision.kind === "fulfilled") {
      const deliveredContent = input.extractDeliveredContent(orderResult);
...
export async function reconcileSupplierExistingCandidate(input) {
  ...
  if (!requestId || !input.candidate.binding || input.candidate.bindingInvalid) {
    throw new Error("SUPPLIER_RECONCILIATION_CANDIDATE_INVALID");
  }
  ...
  if (
    ...
    throw new Error("SUPPLIER_RECONCILIATION_STATE_CHANGED");
  }
  ...
  const deliveredContent = input.extractDeliveredContent(order);
  if (!deliveredContent) throw new Error("SUPPLIER_RECONCILIATION_DELIVERY_MISSING");
  ...
  return { ok: true, orderCode: order.orderCode, requestId, deliveredCount: input.countDelivered(order) };
}
