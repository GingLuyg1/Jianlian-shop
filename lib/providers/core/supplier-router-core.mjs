...
export async function executeSupplierHandlers(handlers, invoke) {
  if (!Array.isArray(handlers) || typeof invoke !== "function") {
    throw new Error("SUPPLIER_ROUTER_EXECUTION_INVALID");
  }
  const summary = zeroSupplierSummary();
  for (const handler of handlers) {
    addSupplierSummary(summary, await invoke(handler));
  }
  return summary;
}
