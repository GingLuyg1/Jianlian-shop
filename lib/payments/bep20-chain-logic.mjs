...
export function decideBep20TransferStatus(input) {
  ...
  // The quoted rate is frozen into expectedRawAmount when the session is created.
  // Its cache TTL only limits creating new snapshots; it is not a payment deadline.
  const deadlines = [input.orderPaymentExpiresAt, input.paymentSessionExpiresAt, input.sessionExpiresAt]
    .filter(Boolean)
  ...
  if (raw > expected) return "overpaid";
  return "verified";
}
...
export function shouldPrefillBep20TxHash(input) {
  const txHash = String(input?.submittedTxHash ?? "").trim();
  if (!txHash) return false;

  const status = String(input?.status ?? "").trim();
  if (["confirming", "verified", "completing", "payment_failed", "manual_review", "paid"].includes(status)) {
    return true;
  }

  if (status === "submitted" && String(input?.failureReason ?? "").trim()) {
    return false;
  }
