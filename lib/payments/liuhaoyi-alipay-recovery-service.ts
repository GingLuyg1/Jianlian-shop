...
export type AlipayRecoveryResult = {
...
export async function runLiuhaoyiAlipayRechargeRecovery(
  input: { sessionNo: string; execute?: boolean; queryTimeoutMs?: number },
  client?: SupabaseClient,
): Promise<AlipayRecoveryResult> {
  const sessionNo = String(input.sessionNo ?? "").trim();
  if (!/^PS[A-Za-z0-9_-]{8,157}$/.test(sessionNo)) throw new Error("ALIPAY_RECOVERY_SESSION_INVALID");
  const execute = input.execute === true;
  const service = client ?? getSupabaseServiceRoleClient();
  if (!service) throw new Error("ALIPAY_RECOVERY_SERVICE_UNAVAILABLE");

  const { data: sessionRow, error: sessionError } = await service
    .from("payment_sessions").select(SESSION_SELECT).eq("session_no", sessionNo).maybeSingle();
  ...
  if (session.provider !== "liuhaoyi" || session.channelCode !== "alipay") {
    throw new Error("ALIPAY_RECOVERY_SESSION_PROVIDER_MISMATCH");
  }
  ...
  if (recharge) {
    const { count, error: ledgerError } = await service
      .from("balance_transactions").select("id", { count: "exact", head: true })
      .eq("business_type", "account_recharge").eq("business_id", recharge.rechargeNo)
      .eq("status", "completed");
  ...
  const decision = evaluateLiuhaoyiAlipayRechargeRecovery({ session, recharge, provider, ledgerCount });
  ...
  const paidWithinExpiry = Boolean(recharge) && Number.isFinite(paidAtMs)
    && paidAtMs <= Date.parse(String(session.expiresAt ?? ""))
    && paidAtMs <= Date.parse(String(recharge?.expiresAt ?? ""));
  const localAlreadyCredited = session.localStatus === "paid" || Boolean(recharge?.completedAt)
    || Number(recharge?.creditedAmount ?? 0) !== 0 || ledgerCount > 0;
  ...
  return {
    mode: execute ? "execute" : "dry_run", sessionNo, sessionFound: true,
    rechargeFound: Boolean(recharge), providerFound: provider.found, providerPaid,
    providerTypeMatch: provider.type === "alipay", amountMatch, paidWithinExpiry,
    localAlreadyCredited, ledgerCount, eligible: decision.eligible,
    wouldComplete: decision.eligible, completed, idempotent,
    manualReview: decision.manualReview, reason: decision.reason,
  };
}
...
function normalizeSession(row: Record<string, unknown>) {
  return {
    ...
    localAmount: row.payable_amount, currency: String(row.currency), expiresAt: text(row.expires_at),
...
async function persistManualReviewEvidence(
  ...
  const { error } = await service.from("payment_reconciliations").upsert({
    reconciliation_no: `REC${checkedAt.replace(/\D/g, "").slice(0, 14)}ALIMANUAL`,
    payment_session_id: session.id, business_type: "recharge",
    business_id: session.businessNo ?? session.businessId, channel_code: "alipay",
    provider: "liuhaoyi", local_status: session.localStatus, provider_status: "paid",
    local_amount: session.localAmount, provider_amount: provider.amount, currency: "CNY",
    result: "manual_review", difference_type: "provider_paid_local_unpaid", risk_level: "high",
    provider_trade_no: tradeNo, local_trade_no: session.localTradeNo,
    error_code: `ALIPAY_RECOVERY_${reason.toUpperCase()}`,
    error_message: "Provider reports paid, but automatic account recharge recovery was not safe.",
    provider_summary: { found: true, paid: true, type: provider.type,
      tradeNoPresent: Boolean(tradeNo), endtime: provider.endtime },
    checked_at: checkedAt, recovery_action: null, recovery_status: "manual_review",
    dedupe_key: `alipay-recovery:${session.id}:${reason}:${tradeNo ?? "missing"}`,
    updated_at: checkedAt,
  }, { onConflict: "dedupe_key" });
  if (error) throw error;
}

function emptyResult(sessionNo: string, execute: boolean, reason: string): AlipayRecoveryResult {
  return { mode: execute ? "execute" : "dry_run", sessionNo, sessionFound: false,
    rechargeFound: false, providerFound: false, providerPaid: false, providerTypeMatch: false,
    amountMatch: false, paidWithinExpiry: false, localAlreadyCredited: false, ledgerCount: 0,
    eligible: false, wouldComplete: false, completed: false, idempotent: false,
    manualReview: false, reason };
}
