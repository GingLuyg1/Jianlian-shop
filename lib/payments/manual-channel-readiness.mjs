...
// Historical defaults are used only when a channel row has no provider identity.
// They do not constrain the provider of a configured or already-created payment.
...
// Compatibility is independent of the default. An adapter must still be registered,
// configured and advertise supportsCreate before a channel can create a payment.
export const PAYMENT_PROVIDER_CHANNEL_SUPPORT = Object.freeze({
  liuhaoyi: ["alipay", "wechat"],
  generic_api: ["alipay", "wechat"],
  binance: ["binance_pay"],
  crypto_address: ["usdt_trc20", "usdt_bep20"],
});
...
export function isChannelProviderCompatible(channel, provider) {
  return isKnownPaymentChannelCode(channel)
    && typeof provider === "string"
    && Object.prototype.hasOwnProperty.call(PAYMENT_PROVIDER_CHANNEL_SUPPORT, provider)
    && PAYMENT_PROVIDER_CHANNEL_SUPPORT[provider].includes(channel);
}
...
export function getPaymentChannelValidationError(input = {}) {
  ...
  if (input.provider === "liuhaoyi" && feeRate !== 0) {
    return `${input.channel ?? "channel"} must not add a local Liuhaoyi fee`;
  }
  ...
  if (input.provider === "liuhaoyi" && maximumAmount > 2000) {
    return `${input.channel ?? "channel"} exceeds the Liuhaoyi maximum amount`;
  }
