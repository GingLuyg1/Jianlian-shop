...
export const watcherExecutionEnabled = (args, env) =>
  args.includes("--execute") && watcherEnabled(env.LIUHAOYI_WECHAT_WATCHER_EXECUTE_ENABLED);
...
export function candidateMode(row, nowMs, execute) {
  ...
  if (row?.status !== "pending" || !Number.isFinite(created)
    ...
    || !Number.isFinite(expiry) || expiry <= nowMs) return "skip";
  // Leave a small margin for the final DB expiry guard and network round trips.
  if (expiry <= nowMs + 5_000) return "dry_run";
...
function candidateUrl(base, nowMs, limit, offset = 0) {
  ...
  for (const [key, value] of Object.entries({
    select: "session_no,status,created_at,expires_at", provider: "eq.liuhaoyi", channel_code: "eq.wechat",
...
export async function runLiuhaoyiWechatWatcher({
  ...
  const baseRecord = { event: "wechat_recovery_watcher", run_id: runId, started_at: new Date(nowMs).toISOString() };
  ...
  if (args.includes("--execute") && !execute) {
    emit({ ...baseRecord, status: "execute_not_enabled" }); return { status: "execute_not_enabled", processed: 0 };
  }
  ...
  try {
    ...
    candidates = newest.rows.slice(0, eligibleCount > WATCHER_BATCH_SIZE ? WATCHER_BATCH_SIZE - 1 : WATCHER_BATCH_SIZE);
    if (eligibleCount > WATCHER_BATCH_SIZE) {
      // Reserve three slots for newest orders, rotate the fourth through older backlog.
  ...
  for (const row of candidates) {
    ...
    emit({ event: "wechat_recovery_session", run_id: runId, session_no: sessionNo, mode,
