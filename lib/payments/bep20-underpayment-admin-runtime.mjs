...
export function rawAmountMatchesDecimal(rawAmount, decimalAmount, decimals) {
  const rawText = String(rawAmount ?? "").trim();
  const amountText = String(decimalAmount ?? "").trim();
  ...
  const fraction = match[2] ?? "";
  if (fraction.length > parsedDecimals) return false;
  const expectedRaw = BigInt(`${match[1]}${fraction}`)
    * 10n ** BigInt(parsedDecimals - fraction.length);
  return BigInt(rawText) === expectedRaw;
}

export function evaluateAdminUnderpaymentEligibility(input) {
  const blockingReasons = Object.entries(input.checks ?? {})
    .filter(([, valid]) => valid !== true)
    .map(([name]) => name);
  const manualEligible = blockingReasons.length === 0;
  const manualBeforeDeadline = manualEligible && input.expired !== true;
  return {
    blockingReasons,
    manualEligible,
    manualBeforeDeadline,
    automaticEligible:
      manualEligible
      && input.expired === true
      && input.authorityCandidate === true,
  };
}
