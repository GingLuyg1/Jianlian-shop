...
export function selectLiuhaoyiPaymentArtifact(payload) {
  const paymentUrl = safeHttpsUrl(payload?.payurl)
    ?? safeHttpsUrl(payload?.qrcode)
    ?? safePaymentDeepLink(payload?.urlscheme);
  return {
...
export function normalizeLiuhaoyiSessionPresentation(input) {
  ...
  const legacyPaymentUrl = isLiuhaoyiCheckout && !paymentUrl ? safeHttpsUrl(qrCodeUrl) : undefined;

  if (legacyPaymentUrl) {
    return { paymentType: "redirect", paymentUrl: legacyPaymentUrl, qrCodeUrl: undefined };
  }
  return {
    paymentType: input?.paymentType === "qrcode" || input?.paymentType === "address"
      ? input.paymentType
      : "redirect",
    paymentUrl,
    qrCodeUrl,
