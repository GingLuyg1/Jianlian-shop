...
begin
  ...
  if not found then raise exception 'Order linked to refund does not exist'; end if;

  if p_action = 'approve_balance' and (coalesce(v_order.payment_method, '') <> 'balance' or coalesce(v_refund.refund_method, '') <> 'balance') then
    raise exception 'External payment refunds cannot be completed as balance refunds';
  end if;

  if p_action = 'complete_external' and (coalesce(v_order.payment_method, '') = 'balance' or coalesce(v_refund.refund_method, '') = 'balance') then
    raise exception 'Balance payment refunds must use the balance refund flow';
  end if;
  ...
  if p_action = 'reject' then
    ...
    if coalesce(nullif(btrim(coalesce(p_provider_refund_id, v_refund.provider_refund_id, '')), ''), '') = '' then
      raise exception '璇峰～鍐欏閮ㄦ笭閬撲汉宸ラ€€娆惧弬鑰冨彿';
    end if;
    ...
    if v_amount <= 0 or v_amount > v_refundable then
      raise exception '鎵瑰噯閲戦瓒呰繃褰撳墠鍙€€閲戦';
    end if;
    ...
    if found then
      ...
      if not found then raise exception '鐢ㄦ埛璧勬枡涓嶅瓨鍦?; end if;
  ...
  values (v_order.id, v_order.status, (select status from public.orders where id = v_order.id), v_admin_id, 'admin', '閫€娆惧鐞嗭細' || coalesce(v_next_status, v_refund.status))
  ...
  values (
    ...
    case coalesce(v_next_status, v_refund.status)
      when 'succeeded' then '閫€娆惧凡瀹屾垚'
      when 'rejected' then '閫€娆剧敵璇疯鎷掔粷'
      when 'failed' then '閫€娆惧鐞嗗け璐?
      when 'processing' then '閫€娆惧鐞嗕腑'
      else '閫€娆剧姸鎬佸凡鏇存柊'
    end,
    coalesce(nullif(v_refund.user_visible_note, ''), '鎮ㄧ殑閫€娆剧敵璇风姸鎬佸凡鏇存柊锛岃鏌ョ湅璁㈠崟璇︽儏銆?),
