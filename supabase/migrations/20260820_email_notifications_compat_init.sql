-- Production-compatible initialization for Jianlian email notifications.
-- This migration intentionally does NOT redefine any administrator authorization function.
-- Safe to run repeatedly after the preconditions below are satisfied.

do $$
declare
  audit_constraint text;
  unknown_allowed_modules text;
  missing_required_modules text;
begin
  if to_regclass('public.profiles') is null then
    raise exception 'email notifications require public.profiles';
  end if;
  if to_regprocedure('public.is_super_admin_user(uuid)') is null then
    raise exception 'email notifications require public.is_super_admin_user(uuid)';
  end if;
  if to_regclass('public.admin_audit_logs') is null then
    raise exception 'email notifications require public.admin_audit_logs';
  end if;
  if to_regprocedure('gen_random_uuid()') is null then
    raise exception 'email notifications require gen_random_uuid()';
  end if;
  ...
  where c.conrelid = 'public.admin_audit_logs'::regclass
    and c.conname = 'admin_audit_logs_module_check'
    and c.contype = 'c';

  if audit_constraint is null then
    raise exception 'email notifications require admin_audit_logs_module_check';
  end if;

  with allowed(module) as (
    select distinct (m)[1]
    from regexp_matches(audit_constraint, '''([^'']+)''::text', 'g') as m
  )
  select string_agg(module, ', ' order by module)
    into unknown_allowed_modules
  from allowed
  where module not in (
    'payments','recharges','orders','users','products','categories',
    'inventory','delivery','settings','system','privacy','notifications'
  );

  if unknown_allowed_modules is not null then
    raise exception 'admin_audit_logs_module_check has unsupported allowed modules: %', unknown_allowed_modules;
  end if;

  with required(module) as (
    select unnest(array[
      'payments','recharges','orders','users','products','categories',
      'inventory','delivery','settings','system','privacy'
    ]::text[])
  ),
  allowed(module) as (
    select distinct (m)[1]
  ...
  select string_agg(required.module, ', ' order by required.module)
    into missing_required_modules
  from required
  left join allowed using (module)
  where allowed.module is null;

  if missing_required_modules is not null then
    raise exception 'admin_audit_logs_module_check is missing required modules: %', missing_required_modules;
  end if;
end;
$$;
...
do $$
declare
  unknown_modules text;
begin
  select string_agg(distinct module, ', ' order by module)
    into unknown_modules
  ...
  if unknown_modules is not null then
    raise exception 'admin_audit_logs contains unsupported module values: %', unknown_modules;
  end if;

  alter table public.admin_audit_logs
    drop constraint admin_audit_logs_module_check;
