...
- `20260728` 与 `20260729` 已执行；`20260730_bep20_underpayment_confirmation_state.sql` 不得修改、重命名或重复执行。历史 `confirmed_at` 回填已完成，只读 postcheck 已通过。
- 当前本地分支新增超级管理员列表、详情、只读预检、双重确认与幂等结算体验；普通用户订单和余额流水会明确标识“欠额已转余额”，避免误示为商品支付成功。
- GET 与默认 POST 均为只读；真实写入必须显式 `action=settle`、`dry_run=false`，并由服务端绑定超级管理员和 service-role。
- 欠额余额结算尚未针对任何真实订单执行；自动定时结算尚未配置；本地实现尚未部署。
