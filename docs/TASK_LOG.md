...
- PR #14 已合并：原始提交 `53b2b87333fd16c74c4e67fbfcd6c4f3a971ddf7`，merge commit `0a76d1051057d2bac6524a9eb5018f226f198ad8`。
- Migration：`supabase/migrations/20260728230700_bep20_phase1_privilege_hardening.sql`。
- 用户在测试项目 Jianlian-shop-test / `czuoivbfxzachiobdohw` 通过 SQL Editor 手动执行，结果为 `Success. No rows returned`。
- 用户在正式项目 Jianlian-shop / `qvbovrvybirscaurwuov` 通过 SQL Editor 手动执行，结果为 `Success. No rows returned`。
- 两个环境的最终 postcheck 均通过：`target_table_count=2`、`rls_disabled_table_count=0`、`unexpected_public_or_anon_table_privilege_count=0`、`unexpected_authenticated_nonselect_count=0`、`missing_authenticated_select_count=0`、`missing_service_role_table_privilege_count=0`、`unexpected_client_column_acl_count=0`、`target_function_count=1`、`unexpected_function_owner_count=0`、`not_security_definer_count=0`、`unexpected_search_path_count=0`。
- 函数执行权限最终为：`public_can_execute=false`、`anon_can_execute=false`、`authenticated_can_execute=false`、`service_role_can_execute=true`。
- 本次权限加固未修改订单、余额、支付状态或链上交易业务数据；未部署；自动结算保持 Disabled。
- 该 Migration 不应重复执行。SQL Editor 手动执行不会写入 Supabase Migration History；禁止使用 `supabase db push`、`migration up`、`migration repair` 或 `db reset --linked`。
...
- 增加超级管理员欠额候选列表、详情、只读预检、订单号确认、不可撤销勾选、二次确认和稳定 request ID。
- 加固内部/管理员 API：GET 与默认请求只读，真实结算必须显式进入写路径；操作员身份仅来自服务端认证上下文。
- 用户订单、订单抽屉和账户余额流水增加欠额转余额最终状态；后台统一显示“欠额款已转入用户余额，原订单已取消”。
- 增加单一 psql 会话 Migration runner、执行手册和安全失败测试；`20260730` Migration 内容保持不变。
- 正式事实：历史 `confirmed_at` 回填和只读 postcheck 已完成；欠额余额结算与自动定时结算均未执行。
- 本轮未连接正式数据库，未执行 SQL/RPC/API，未处理真实订单，未部署，也未修改环境变量。
