...
Remaining coupling: the finite provider registry/readiness definitions; Liuhaoyi-specific submit-form reconstruction, protocol amount and expiry guards; and Liuhaoyi-specific recovery implementation. A new non-submit acquirer should need only an adapter/protocol helper, registry capability/config entry, env-name documentation, tests, and controlled channel configuration. A new submit-mode acquirer additionally needs a safe server-side form validator. None of this establishes Production callback ingress or readiness.
