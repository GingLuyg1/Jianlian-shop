...
type UserDetail = {
  ...
  summary: { balance: number; totalRecharge: number; totalSpend: number; orderCount: number; rechargeCount: number; transactionCount: number; deliveryCount: number };
        ...
        {error ? <AdminErrorState title="用户详情加载失败" description={error} onRetry={() => void loadDetail()} /> : loading && !detail ? <AdminTableSkeleton rows={10} /> : detail && profile ? (
          <div className="space-y-4">
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
              ...
              <Section title="账户摘要"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><Info label="当前余额" value={money(detail.summary.balance)} /><Info label="累计充值（最近记录）" value={money(detail.summary.totalRecharge)} /><Info label="累计消费（最近记录）" value={money(detail.summary.totalSpend)} /><Info label="账户 / 风险状态" value={`${ACCOUNT_LABELS[profile.accountStatus] ?? profile.accountStatus} / ${RISK_LABELS[profile.riskStatus] ?? profile.riskStatus}`} /></div></Section>
            </div>

            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <RelatedLink icon={ShoppingBag} label="关联订单" count={detail.summary.orderCount} href={`/admin/orders?search=${relatedSearch}`} />
              <RelatedLink icon={CreditCard} label="关联支付" href={`/admin/payments?search=${relatedSearch}`} />
              <RelatedLink icon={WalletCards} label="关联充值" count={detail.summary.rechargeCount} href={`/admin/recharges?search=${relatedSearch}`} />
              <RelatedLink icon={ShieldAlert} label="关联风险" count={detail.riskEvents.length} href={`/admin/risk?search=${encodeURIComponent(userId)}`} />
            </section>
            ...
            <Section title="风险事件"><RecordList rows={detail.riskEvents} columns={[["ruleCode", "规则"], ["riskLevel", "等级"], ["riskScore", "分数"], ["businessType", "业务"], ["summary", "摘要"], ["status", "状态"], ["lastSeenAt", "最后发现"]]} detailBase="/admin/risk" /></Section>
...
function RelatedLink({ icon: Icon, label, count, href }: { icon: typeof ShoppingBag; label: string; count?: number; href: string }) { return <Link href={href} className="group flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-primary/30 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><div className="flex items-center gap-3"><span className="rounded-lg bg-slate-100 p-2 text-slate-500 group-hover:text-primary"><Icon className="h-4 w-4" /></span><span className="font-medium text-slate-900">{label}</span></div><span className="text-sm tabular-nums text-slate-500">{count ?? "查看"}</span></Link>; }
