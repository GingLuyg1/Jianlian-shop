      ...
      {open && keyword.trim() ? (
        <div className="absolute left-0 right-0 top-11 z-50 max-h-[560px] overflow-hidden rounded-[var(--admin-v2-surface-radius)] border border-[var(--admin-v2-border)] bg-[var(--admin-v2-surface)] shadow-lg">
          {error ? (
            ...
            <div className="px-4 py-5 text-sm text-[var(--admin-v2-text-muted)]">正在搜索...</div>
          ) : payload && payload.total === 0 ? (
            <div className="px-4 py-8 text-center text-sm text-[var(--admin-v2-text-muted)]">
              <div className="font-semibold text-[var(--admin-v2-text-primary)]">未找到相关业务记录</div>
                      ...
                      {group.results.map((result) => {
                        ...
                        return (
                          <Link
                            ...
                            className={cn(
                              "block rounded-[var(--admin-v2-control-radius)] px-3 py-2 text-sm transition-colors",
                              index === activeIndex ? "bg-[var(--admin-v2-selected)]" : "hover:bg-[var(--admin-v2-surface-muted)]"
                            )}
                          >
