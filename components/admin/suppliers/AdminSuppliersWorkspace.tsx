        ...
        {activeArea === "overview" ? (
          ...
          <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            {productsError ? <AdminErrorState title="供应商商品目录不可用" description={productsError.message} action={<div className="text-left text-red-700"><SupplierErrorDetails error={productsError} /></div>} /> : productsLoading ? <AdminTableSkeleton rows={8} /> : products.length === 0 ? <AdminEmptyState icon={<PackageSearch className="h-5 w-5" />} title={query.trim() ? "没有匹配的供应商商品" : "供应商商品目录为空"} description="请调整搜索关键词或稍后刷新。" /> : (
              <div className="min-h-0 flex-1 overflow-auto"><table className="min-w-[980px] w-full table-fixed text-left text-sm"><thead className="sticky top-0 z-10 bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">供应商商品</th><th className="px-4 py-3 text-center">ID</th><th className="px-4 py-3 text-center">价格</th><th className="px-4 py-3 text-center">库存</th><th className="px-4 py-3 text-center">销量</th><th className="px-4 py-3 text-center">自动交付</th><th className="px-4 py-3 text-center">排序 ID</th><th className="px-4 py-3 text-center">操作</th></tr></thead><tbody className="divide-y divide-slate-100">{products.map((product) => <tr key={product.id} className="hover:bg-slate-50/70"><td className="min-w-0 px-4 py-3"><div className="flex min-w-0 items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-100"><PackageSearch className="h-4 w-4 text-slate-400" /></div><div className="min-w-0"><div className="truncate font-medium text-slate-950" title={product.title}>{product.title}</div><div className="truncate text-xs text-slate-500" title={product.cover ?? ""}>Cover：{product.cover || "—"}</div></div></div></td><td className="px-4 py-3 text-center font-mono text-xs">{product.id}</td><td className="px-4 py-3 text-center tabular-nums">¥{product.price}</td><td className="px-4 py-3 text-center tabular-nums">{product.stock}</td><td className="px-4 py-3 text-center tabular-nums">{product.sales}</td><td className="px-4 py-3 text-center"><Badge variant={product.isAuto ? "secondary" : "outline"}>{product.isAuto ? "是" : "否"}</Badge></td><td className="px-4 py-3 text-center">{product.sortId ?? "—"}</td><td className="px-4 py-3 text-center whitespace-nowrap"><Button variant="outline" size="sm" onClick={() => openProductDetail(product.id)}>查看详情</Button></td></tr>)}</tbody></table></div>
            )}
          </section>
        )}
      </div>
