    ...
    <AdminDetailDrawer title={selected.payment_no} eyebrow={isRechargePage ? "充值详情" : "支付详情"} onClose={onClose} className="max-w-[760px]">
          ...
          {!detailLoading && !detailError && detail ? (
            <div className="space-y-4">
              ...
              <div className="grid gap-px overflow-hidden rounded-[var(--admin-v2-surface-radius)] border border-[var(--admin-v2-border)] bg-white sm:grid-cols-3">
...
function DrawerSummaryItem({ label, value }: { label: string; value: string }) { return <div className="min-w-0 bg-white px-4 py-3"><div className="text-xs text-[var(--admin-v2-text-muted)]">{label}</div><div className="mt-0.5 truncate text-sm font-semibold text-[var(--admin-v2-text-primary)]" title={value}>{value || "—"}</div></div>; }
function DetailGroup({ title, rows }: { title: string; rows: Array<[string, string, (() => void)?]> }) { return <section className="min-w-0 overflow-hidden rounded-[var(--admin-v2-surface-radius)] border border-[var(--admin-v2-border)] bg-white"><div className="px-4 pt-4"><h2 className="text-sm font-semibold text-[var(--admin-v2-text-primary)]">{title}</h2></div><AdminInfoGrid className="px-4 pb-3 sm:px-4 sm:pb-4">{rows.map(([label, value, onCopy]) => <AdminInfoItem key={label} label={label} value={onCopy ? <button type="button" onClick={onCopy} className="min-h-11 break-all text-left text-[var(--admin-v2-primary)] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--admin-v2-primary)] sm:min-h-0">{value || "—"}</button> : <span className="break-all">{value || "—"}</span>} />)}</AdminInfoGrid></section>; }
