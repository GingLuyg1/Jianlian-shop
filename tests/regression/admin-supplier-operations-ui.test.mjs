...
test("supplier operations UI uses existing admin APIs without changing fulfillment authority", () => {
  ...
  assert.match(bindingSheet, /disabled=\{saving \|\| !detail \|\| !detail\.isAuto \|\| \(detail\.isSku && !supplierSku\.trim\(\)\) \|\| missingRequiredInputs\.length > 0\}/);
  ...
  const productWrite = bindingRoute.indexOf("const stockResolution = resolveDajuEffectiveStock");
