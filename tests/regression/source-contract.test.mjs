...
test("BEP20 underpayment migration source specifies atomic wallet credit without payment completion", () => {
  const migration = file("supabase/migrations/20260728_bep20_underpayment_wallet_credit.sql");
  const settlementFunction = migration.match(
  ...
  assert.match(migration, /create table public\.bep20_underpayment_dispositions/i);
  assert.match(migration, /chain_session_id uuid primary key/);
  assert.match(migration, /order_id uuid not null unique/);
  assert.match(migration, /payment_id uuid not null unique/);
  assert.match(migration, /payment_session_id uuid not null unique/);
  assert.match(migration, /balance_transaction_id uuid not null unique/);
  assert.match(migration, /request_id text not null unique/);
  assert.match(migration, /shortfall_usdt = expected_usdt - received_usdt/);
  assert.match(migration, /settlement_source in \('automatic_service','manual_admin'\)/);
  assert.match(migration, /processed_by uuid references auth\.users\(id\) on delete restrict/);
  assert.doesNotMatch(migration, /processed_by uuid references auth\.users\(id\) on delete set null/);
  assert.match(migration, /a\.attname = 'processed_by'[\s\S]*?c\.confdeltype in \('a','r'\)/);
  assert.match(migration, /alter table public\.bep20_underpayment_dispositions enable row level security/i);
  assert.match(migration, /revoke all privileges on table public\.bep20_underpayment_dispositions[\s\S]*?from public, anon, authenticated/i);

  assert.match(migration, /create function public\.settle_bep20_underpayment_to_wallet\(/i);
  assert.match(migration, /security definer\s+set search_path = public/i);
  assert.match(migration, /BEP20_UNDERPAYMENT_SERVICE_ROLE_REQUIRED/);
  assert.match(migration, /pg_advisory_xact_lock/);
  assert.match(settlementFunction, /v_chain\.status <> 'underpaid'/);
  assert.match(settlementFunction, /v_payment_session\.business_id is distinct from v_order\.id/);
  assert.match(settlementFunction, /v_payment_session\.user_id is distinct from v_order\.user_id/);
  assert.match(settlementFunction, /v_order_payment\.payment_session_id is null/);
  assert.match(settlementFunction, /v_order_payment\.payment_session_id is distinct from v_payment_session\.id/);
  assert.match(settlementFunction, /v_order_payment\.status is distinct from 'under_review'/);
  assert.match(settlementFunction, /v_transaction\.confirmation_count is null/);
  assert.match(settlementFunction, /v_transaction\.normalized_amount is null/);
  assert.match(settlementFunction, /v_provider_transaction_id := v_tx_hash \|\| ':' \|\| v_transaction\.log_index::text/);
  assert.match(settlementFunction, /upper\(v_order\.currency\) is distinct from 'CNY'/);
  assert.match(settlementFunction, /lower\(v_order\.payment_method\) is distinct from 'usdt_bep20'/);
  assert.match(settlementFunction, /v_chain\.order_amount is distinct from v_order\.total_amount/);
  assert.match(settlementFunction, /v_payment_session\.business_no is distinct from v_order\.order_no/);
  assert.match(settlementFunction, /lower\(v_payment_session\.wallet_address\) is distinct from lower\(v_chain\.receive_address\)/);
  assert.match(settlementFunction, /v_order_payment\.amount is distinct from v_order\.total_amount/);
  assert.match(settlementFunction, /v_order_payment\.order_amount is distinct from v_order\.total_amount/);
  assert.match(settlementFunction, /v_chain\.confirmed_at is null/);
  assert.match(settlementFunction, /v_payment_session\.payable_amount is distinct from v_chain\.expected_amount/);
  assert.match(settlementFunction, /v_order_payment\.payable_amount is distinct from v_chain\.expected_amount/);
  assert.match(settlementFunction, /v_order_payment\.received_amount is distinct from v_chain\.confirmed_amount/);
  assert.doesNotMatch(settlementFunction, /round\((?:v_payment_session|v_order_payment)\.(?:payable_amount|received_amount), 6\)/);
  assert.match(settlementFunction, /v_order\.payment_expires_at is null/);
  assert.match(settlementFunction, /v_payment_session\.expires_at is null/);
  assert.match(settlementFunction, /v_chain\.expires_at is null/);
  assert.match(settlementFunction, /BEP20_UNDERPAYMENT_DEADLINE_INVALID/);
  assert.match(settlementFunction, /v_deadline := least\(/);
  assert.doesNotMatch(settlementFunction, /select min\(x\.deadline\)|unnest\(array/);
  assert.match(migration, /v_chain\.manual_review_decision is not null/);
  assert.match(migration, /v_transaction\.confirmation_count < p_required_confirmations/);
  assert.match(migration, /v_transaction\.block_timestamp > v_deadline/);
  assert.match(migration, /v_received_raw <= 0 or v_received_raw >= v_expected_raw/);
  assert.match(migration, /v_credited_cny := round\(v_received_usdt \* v_chain\.exchange_rate, 2\)/);
  assert.match(migration, /insert into public\.balance_transactions/i);
  assert.match(migration, /insert into public\.bep20_underpayment_dispositions/i);
  assert.match(migration, /public\.release_order_inventory\(v_order\.id/);
  assert.match(migration, /set status = 'cancelled', payment_status = 'failed'/);
  assert.match(migration, /set status = 'expired',[\s\S]*?underpayment_credited_to_wallet/);
  assert.doesNotMatch(settlementFunction, /complete_payment_session\s*\(/i);
  assert.doesNotMatch(settlementFunction, /deliver_digital_order\s*\(/i);
  assert.doesNotMatch(settlementFunction, /update public\.chain_transactions/i);

  assert.match(migration, /return jsonb_build_object\([\s\S]*?'result', 'already_settled'/);
  assert.match(migration, /v_balance_transaction_no := 'BT-BEP20-UNDER-'/);
  assert.match(migration, /tx_hash_claim_retained/);
  assert.doesNotMatch(migration, /insert into public\.bep20_underpayment_dispositions[^;]+\)\s*select\b/i);
  assert.match(migration, /revoke all on function public\.settle_bep20_underpayment_to_wallet[\s\S]*?from public, anon, authenticated/i);
  assert.match(migration, /grant execute on function public\.settle_bep20_underpayment_to_wallet[\s\S]*?to service_role/i);
});

test("BEP20 underpayment candidate listing is expired-only and service-role-only", () => {
  const migration = file("supabase/migrations/20260728_bep20_underpayment_wallet_credit.sql");

  assert.match(migration, /create function public\.list_expirable_bep20_underpayments\(p_limit integer default 50\)/i);
  assert.match(migration, /greatest\(1, least\(coalesce\(p_limit, 50\), 200\)\)/);
  assert.match(migration, /cps\.status = 'underpaid'/);
  assert.match(migration, /cps\.confirmed_raw_amount > 0/);
  assert.match(migration, /cps\.confirmed_raw_amount < cps\.expected_raw_amount/);
  assert.match(migration, /o\.status = 'pending_payment'/);
  assert.match(migration, /o\.payment_status = 'unpaid'/);
  assert.match(migration, /op\.status = 'under_review'/);
  assert.match(migration, /op\.payment_session_id = ps\.id/);
  assert.match(migration, /ps\.payable_amount is not distinct from cps\.expected_amount/);
  assert.match(migration, /op\.payable_amount is not distinct from cps\.expected_amount/);
  assert.match(migration, /op\.received_amount is not distinct from cps\.confirmed_amount/);
  assert.match(migration, /cps\.confirmed_amount > 0/);
  assert.match(migration, /cps\.confirmed_amount < cps\.expected_amount/);
  assert.match(migration, /cps\.confirmed_at is not null/);
  assert.match(migration, /cps\.chain_id = 56/);
  assert.match(migration, /upper\(cps\.network\) = 'BEP20'/);
  assert.match(migration, /o\.payment_expires_at is not null/);
  assert.match(migration, /ps\.expires_at is not null/);
  assert.match(migration, /cps\.expires_at is not null/);
  assert.match(migration, /least\(o\.payment_expires_at, ps\.expires_at, cps\.expires_at\) < now\(\)/);
  assert.doesNotMatch(migration, /round\((?:ps|op)\.(?:payable_amount|received_amount), 6\) = round\(cps\.(?:expected_amount|confirmed_amount), 6\)/);
  assert.doesNotMatch(migration, /cross join lateral[\s\S]*?unnest\(array\[o\.payment_expires_at/);
  assert.match(migration, /not exists \([\s\S]*?bep20_underpayment_dispositions/);
  assert.match(migration, /revoke all on function public\.list_expirable_bep20_underpayments[\s\S]*?from public, anon, authenticated/i);
  assert.match(migration, /grant execute on function public\.list_expirable_bep20_underpayments[\s\S]*?to service_role/i);
  assert.match(migration, /alter function public\.settle_bep20_underpayment_to_wallet\(uuid,integer,text,text,text,uuid\)[\s\S]*?owner to postgres/i);
  assert.match(migration, /alter function public\.list_expirable_bep20_underpayments\(integer\)[\s\S]*?owner to postgres/i);
  assert.match(migration, /pg_catalog\.pg_get_userbyid\(p\.proowner\) is distinct from 'postgres'/);
  assert.match(migration, /or not p\.prosecdef/);
...
test("BEP20 underpayment internal and admin routes preserve trust boundaries", () => {
  ...
  assert.match(service, /service\.rpc\("settle_bep20_underpayment_to_wallet"/);
  assert.match(service, /service\.rpc\("list_expirable_bep20_underpayments"/);
  assert.match(service, /source: "automatic_service"/);
  assert.match(service, /irreversibleConfirmed: false/);
  assert.match(service, /p_irreversible_confirmed: options\.irreversibleConfirmed/);
  assert.match(service, /settleBep20UnderpaymentCandidates/);
  assert.doesNotMatch(service, /BSC_REQUIRED_CONFIRMATIONS \?\? 12/);
  assert.match(service, /readBep20UnderpaymentConfirmations\(process\.env\.BSC_REQUIRED_CONFIRMATIONS\)/);
  assert.match(runtime, /BEP20_UNDERPAYMENT_CONFIRMATION_CONFIG_INVALID/);
  assert.match(runtime, /Number\.isInteger\(parsed\)/);
  assert.match(runtime, /parsed < 1 \|\| parsed > 1000/);
  assert.match(service, /CRON_SECRET \|\| process\.env\.INTERNAL_JOB_SECRET/);
  assert.doesNotMatch(service, /submitted_tx_hash|tx_hash/);

  assert.match(internalRoute, /checkRateLimit\("internal_task"/);
  assert.match(internalRoute, /listExpirableBep20UnderpaymentsDryRun/);
  assert.match(internalRoute, /processExpiredBep20Underpayments/);
  assert.match(internalRoute, /dry_run/);
  assert.match(internalRoute, /checkRequestSize\(request, 8 \* 1024\)/);
  assert.match(internalRoute, /status: result\.ok \? 200 : 503/);
  assert.doesNotMatch(internalRoute, /creditedCny|receivedUsdt|expectedUsdt|exchangeRate|txHash/);
  assert.match(service, /schema cache\|could not find the function\|does not exist/);

  assert.match(adminRoute, /getServerSuperAdminContext\(\)/);
  assert.ok(
    adminRoute.indexOf("getServerSuperAdminContext()") < adminRoute.indexOf("settleBep20Underpayment(sessionId"),
    "super-admin authorization must happen before service-role settlement",
  );
...
test("BEP20 underpayment manual early confirmation replaces only the legacy RPC signature", () => {
  const migration = file("supabase/migrations/20260729_bep20_underpayment_manual_early_confirmation.sql");

  assert.match(migration, /^begin;/m);
  assert.match(migration, /commit;\s*$/i);
  assert.match(migration, /revoke all on function public\.settle_bep20_underpayment_to_wallet\(uuid,integer,text,text,text,uuid\)[\s\S]*?service_role/i);
  ...
  assert.match(migration, /alter function public\.settle_bep20_underpayment_to_wallet\(uuid,integer,text,text,text,uuid,boolean\)[\s\S]*?owner to postgres/i);
  assert.match(migration, /security definer\s+set search_path = public/i);
  assert.match(migration, /revoke all on function public\.settle_bep20_underpayment_to_wallet\(uuid,integer,text,text,text,uuid,boolean\)[\s\S]*?from public, anon, authenticated/i);
  assert.match(migration, /grant execute on function public\.settle_bep20_underpayment_to_wallet\(uuid,integer,text,text,text,uuid,boolean\)[\s\S]*?to service_role/i);
  assert.match(migration, /v_source = 'automatic_service' and p_irreversible_confirmed is distinct from false/);
  ...
  assert.match(migration, /v_manual_before_deadline := v_source = 'manual_admin' and v_now <= v_deadline/);
  assert.match(migration, /v_transaction\.block_timestamp > v_deadline/);
  ...
  assert.match(migration, /'settlement_source', v_source/);
  assert.match(migration, /pg_catalog\.pg_get_userbyid\(p\.proowner\) is distinct from 'postgres'/);
  assert.match(migration, /not p\.prosecdef/);
  assert.match(migration, /array\['search_path=public'\]::text\[\]/);
});

test("BEP20 underpayment database verification is guarded, synthetic, and rollback-only", () => {
  const verification = file("supabase/tests/bep20_underpayment_wallet_credit_verification.sql");

  assert.match(verification, /^begin;/m);
  assert.match(verification, /v_confirm_test_database boolean := false/);
  assert.match(verification, /BEP20_UNDERPAYMENT_TEST_DATABASE_CONFIRMATION_REQUIRED/);
  assert.match(verification, /gen_random_uuid\(\)/);
  assert.match(verification, /public\.settle_bep20_underpayment_to_wallet\(/);
  assert.match(verification, /uuid,integer,text,text,text,uuid,boolean/);
  assert.match(verification, /BEP20_UNDERPAYMENT_TEST_LEGACY_FUNCTION_STILL_EXISTS/);
  assert.match(verification, /negative-automatic-before-deadline/);
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_NOT_EXPIRED'/);
  assert.match(verification, /negative-manual-without-confirmation/);
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_IRREVERSIBLE_CONFIRMATION_REQUIRED'/);
  assert.match(verification, /negative-manual-late-transfer/);
  assert.match(verification, /'manual_admin', v_admin_id, true/);
  assert.match(verification, /"irreversible_confirmed":true,"manual_before_deadline":true,"settlement_source":"manual_admin"/);
  assert.match(verification, /public\.list_expirable_bep20_underpayments\(/);
  assert.match(verification, /confirmed_amount = null/);
  assert.match(verification, /payment_session_id = null/);
  assert.match(verification, /confirmation_count = null/);
  assert.match(verification, /order_id = v_other_order_id/);
  assert.match(verification, /block_timestamp = v_deadline \+ interval '1 second'/);
  assert.match(verification, /set status = 'pending'/);
  assert.match(verification, /set payable_amount = 3\.99/);
  assert.match(verification, /set payable_amount = 3\.999999/);
  assert.match(verification, /set received_amount = 2\.989999/);
  assert.match(verification, /set expires_at = null/);
  assert.match(verification, /set payment_expires_at = null/);
  assert.match(verification, /alter table public\.chain_payment_sessions\s+alter column expires_at drop not null/i);
  assert.match(verification, /alter table public\.chain_payment_sessions\s+alter column expires_at set not null/i);
  const deadlineDropNotNull = verification.indexOf("alter column expires_at drop not null");
  const deadlineLoop = verification.indexOf("foreach v_deadline_field");
  const deadlineSetNotNull = verification.indexOf("alter column expires_at set not null");
  const deadlinePassNotice = verification.indexOf("PASS 02C:");
  assert.ok(deadlineDropNotNull >= 0 && deadlineDropNotNull < deadlineLoop);
  ...
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_CLAIM_INVALID'/);
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_LATE_TRANSFER'/);
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_PAYMENT_STATE_INVALID'/);
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_PAYMENT_SNAPSHOT_MISMATCH'/);
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_ORDER_SNAPSHOT_MISMATCH'/);
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_AUTOMATIC_OPERATOR_FORBIDDEN'/);
  assert.match(verification, /v_error is distinct from 'BEP20_UNDERPAYMENT_SUPER_ADMIN_REQUIRED'/);
  assert.match(verification, /credited_cny'\)::numeric <> 21\.53/);
  assert.match(verification, /status = 'cancelled' and payment_status = 'failed'/);
  assert.match(verification, /public\.order_deliveries where order_id = v_order_id/);
  assert.match(verification, /v_result->>'result' <> 'already_settled'/);
  assert.match(verification, /'automatic_service', v_admin_id/);
  assert.match(verification, /'manual_admin', v_admin_id/);
  assert.match(verification, /v_manual_order_id uuid := gen_random_uuid\(\)/);
  assert.match(verification, /settlement_source = 'manual_admin' and processed_by = v_admin_id/);
  assert.match(verification, /target_id = v_manual_chain_session_id::text and result = 'success'/);
  assert.match(verification, /target_id = v_chain_session_id::text and result = 'success'/);
