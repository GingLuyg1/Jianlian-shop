...
test("systemd service uses root-only env, bounded execution and a separate flock", () => {
  ...
  assert.match(service, /exec \/usr\/bin\/node "\$\$JIANLIAN_RELEASE_DIR\/scripts\/ops\/liuhaoyi-alipay-recharge-watcher\.mjs" --watcher-lock-held --execute/);
  ...
  assert.doesNotMatch(service, /JIANLIAN_NODE_BINARY|\/root\//);
