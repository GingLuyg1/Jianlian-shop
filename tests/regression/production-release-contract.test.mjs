...
test("production release script enforces immutable release and recovery gates", () => {
  ...
  assert.match(script, /readlink -f "\/proc\/\$pid\/cwd"/);
  assert.match(script, /activate_release_fresh "\$target"[\s\S]*restore_previous_release "\$current"/);
  assert.match(script, /restore_previous_release\(\)[\s\S]*activate_release_fresh "\$previous"[\s\S]*verify_public_endpoints[\s\S]*pm2 save/);
  assert.match(script, /switch_release\(\)[\s\S]*activate_release_fresh "\$target"[\s\S]*verify_public_endpoints[\s\S]*pm2 save/);
