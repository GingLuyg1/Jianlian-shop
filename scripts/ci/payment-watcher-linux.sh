...
grep -Fq 'exec /usr/bin/node "$$JIANLIAN_RELEASE_DIR/scripts/ops/liuhaoyi-alipay-recharge-watcher.mjs" --watcher-lock-held' "$dry_run_service"
grep -Fq -- '--watcher-lock-held' "$dry_run_service"
grep -Fxq 'ProtectHome=true' "$dry_run_service"
! grep -Fq 'JIANLIAN_NODE_BINARY' "$dry_run_service"
! grep -Fq '/root/' "$dry_run_service"
